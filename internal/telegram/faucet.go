...
func (bot TipBot) makeFaucetKeyboard(ctx context.Context, id string) *tb.ReplyMarkup {
	// inlineFaucetMenu := &tb.ReplyMarkup{ResizeReplyKeyboard: true}
	inlineFaucetMenu := &tb.ReplyMarkup{ResizeReplyKeyboard: true}
	btnAcceptInlineFaucet := inlineFaucetMenu.Data(Translate(ctx, "collectButtonMessage"), "confirm_faucet_inline", id)
	btnCancelInlineFaucet := inlineFaucetMenu.Data(Translate(ctx, "cancelButtonMessage"), "cancel_faucet_inline", id)
	inlineFaucetMenu.Inline(
		inlineFaucetMenu.Row(
			btnAcceptInlineFaucet,
			btnCancelInlineFaucet),
	)
